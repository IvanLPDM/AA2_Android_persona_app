...
class Ajustes : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateTheme(backgroundImage, selectorImage)
    ...
    private fun updateTheme(backgroundImage: ImageFilterView, selectorImage: ImageView) {
        if (isDarkMode) {
            backgroundImage.setColorFilter(resources.getColor(R.color.style_2, theme))
            selectorImage.setImageResource(R.mipmap.selector_settings_v2)

        } else {
            backgroundImage.setColorFilter(resources.getColor(R.color.style_1, theme))
            selectorImage.setImageResource(R.mipmap.selector_settings)
        }
    }
}